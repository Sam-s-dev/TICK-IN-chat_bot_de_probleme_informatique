...
import {
  ...
  User, Lock, Mail, Save, AlertCircle, Check, Eye, EyeOff
  ...
  <div class="min-h-screen bg-secondary flex">
    <aside :class="['fixed lg:static inset-y-0 left-0 z-40 w-64 bg-white border-r border-primary/5 flex flex-col transition-transform duration-300 lg:translate-x-0', sidebarOpen ? 'translate-x-0' : '-translate-x-full']">
      ...
      <nav class="flex-1 p-4 space-y-1">
        ...
        <router-link to="/etudiant/parametres" class="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-primary/10 text-primary text-sm font-semibold"><Settings class="w-4 h-4" /> Paramètres</router-link>
      </nav>
          ...
          <div class="bg-white rounded-xl border border-primary/5 p-6 lg:p-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div class="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-secondary/50 border border-dark/10 text-sm text-muted"><Mail class="w-4 h-4 text-muted" /> {{ profile.email || 'Non renseigné' }}</div>
              </div>
              <div>
                <label class="block text-xs font-semibold text-dark mb-1">Nom d'utilisateur</label>
                <div class="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-secondary/50 border border-dark/10 text-sm text-muted"><User class="w-4 h-4 text-muted" /> {{ profile.username || 'Non renseigné' }}</div>
              </div>
              <div v-if="profile.genre">
                <label class="block text-xs font-semibold text-dark mb-1">Genre</label>
                <div class="px-3.5 py-2.5 rounded-xl bg-secondary/50 border border-dark/10 text-sm text-muted">{{ profile.genre === 'M' ? 'Masculin' : 'Féminin' }}</div>
              </div>
              <div v-if="profile.niveau_etude">
                <label class="block text-xs font-semibold text-dark mb-1">Niveau</label>
                <div class="px-3.5 py-2.5 rounded-xl bg-secondary/50 border border-dark/10 text-sm text-muted">{{ profile.niveau_etude }}</div>
              </div>
              <div v-if="profile.programme" class="sm:col-span-2">
                <label class="block text-xs font-semibold text-dark mb-1">Programme</label>
                <div class="px-3.5 py-2.5 rounded-xl bg-secondary/50 border border-dark/10 text-sm text-muted">{{ profile.programme }}</div>
              </div>
            </div>
          </div>
